...
function Roulette(audio) {
    ...
    function doTween(target) {
        ...
        .staggerFromTo($('.active').find('.description'), 0.3, {
            autoAlpha: 0,
            x: 100,
        },
        {
    ...
    $('.father-name').on('click', function() {
        var fatherIndex = $(this).data('father');
        doTween($(navigationItem[fatherIndex]),
            fatherIndex);
    });
